...
class PublishHook(Hook):
    ...
    def __publish_alembic_cache(self, item, output, work_template, primary_publish_path, 
        ...
        try:
            self.parent.log_debug("Executing command")
            from vermaya.exporters import abcexporter
            exporter = abcexporter.AbcExporter(publish_path)
            exporter.frameRange = [start_frame, end_frame]
            exporter.worldSpace = True
            mainGroup = tank.platform.current_engine().context.entity["name"]
            if not cmds.objExists(mainGroup):
                raise TankError("Failed to export Alembic Cache: %s group can not find." % mainGroup)
            exporter.exportObjects = [mainGroup]
            exporter.main()
        except Exception, e:
    ...
    def __publish_gpu_cache(self, item, output, work_template, primary_publish_path,
                                    sg_task, comment, thumbnail_path, progress_cb):
        ...
        progress_cb(30, "Exporting Gpu cache")
        try:
            self.parent.log_debug("Executing command")
            from vermaya.exporters import gpuexporter
            exporter = gpuexporter.GpuExporter(publish_path)
            exporter.frameRange = [start_frame, end_frame]
            mainGroup = tank.platform.current_engine().context.entity["name"]
            if not cmds.objExists(mainGroup):
                raise TankError("Failed to export Gpu Cache: %s group can not find." % mainGroup)
            exporter.exportObjects = [mainGroup]
            exporter.main()
        except Exception, e:
            raise TankError("Failed to export Gpu Cache: %s" % e)
